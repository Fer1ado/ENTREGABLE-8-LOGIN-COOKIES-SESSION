...
sessionRoute.get("/login", async (req, res) =>{
    ...
    console.log("🚀 ~ sessionRoute.get ~ req.query:", req.query)
    try {
...
sessionRoute.get("/logout", async (req, res) =>{
    ...
    req.session.destroy(error =>{
        if(!error) return res.json({message: "logout Succesfull"}).redirect("/login")
...
sessionRoute.post("/register", async (req, res) =>{
    try {
        ...
        const {  Nombre, Apellido, Email, Password, Telefono, Direccion, Pais, Genero, Edad } = req.body
       ...
       // if(exist) return res.json({ status: "failed", message: "user already registered", })
        ...
        console.log("🚀 ~ sessionRoute.post ~ newUser:", newUser)
